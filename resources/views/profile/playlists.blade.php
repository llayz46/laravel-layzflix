<x-layout title="{{ \Illuminate\Support\Str::title($user->username) }}'s playlists">
            ...
            <div class="flex justify-between">
                <div>
                    @if(count($playlists) > 0)
                        <h2 class="text-base font-semibold leading-8 text-primary-500">All playlists</h2>
                        <p class="text-sm font-medium text-body">{{ \Illuminate\Support\Str::ucfirst($user->username) }}'s playlists</p>
                    @else
                        <h2 class="text-base font-semibold leading-8 text-primary-500">No playlists</h2>
                        <p class="text-sm font-medium text-body">{{ \Illuminate\Support\Str::ucfirst($user->username) }}
                            has no one playlist</p>
                    @endif
                </div>
                            ...
                            <div class="flex w-full">
                                @if($currentPage > 1)
                                    <div class="ml-0 mr-auto">
                                        <a href="{{ route('profile.playlists', ['user' => $user->username, 'page' => $currentPage - 1]) }}"
