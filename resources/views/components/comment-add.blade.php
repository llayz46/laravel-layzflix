        ...
        <form action="{{ route('review.add') }}" class="relative" method="post">
            ...
            <div
                ...
                <textarea rows="3" name="comment" id="comment"
                          ...
                          placeholder="Add your comment..." {{ $errors->has('comment') || $errors->has('note') ? 'autofocus' : '' }}>@if(session('review')) @if(session('review')['comment']) {{ Str::trim(session('review')['comment']) }} @elseif($userReview){{ Str::trim($userReview->comment) }}@endif @endif</textarea>
            </div>
